...
import java.util.stream.Collectors;
        ...
        public class RespuestaController {
            ...
            public List<RespuestaDTO> listaRespuestasEstado(@RequestParam String estado) {
                return rS.listaRespuestasEstado(estado).stream().map(y -> {
                    ModelMapper m = new ModelMapper();
                    return m.map(y, RespuestaDTO.class);
                }).collect(Collectors.toList());
            }
            ...
            public List<RespuestaDTO> listaRespuestaGrado(@RequestParam String grado) {
                return rS.listaRespuestaGrado(grado).stream().map(y -> {
                    ModelMapper m = new ModelMapper();
                    return m.map(y, RespuestaDTO.class);
                }).collect(Collectors.toList());
            }
